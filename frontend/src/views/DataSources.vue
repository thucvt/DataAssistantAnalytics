...
// Generic paste-token save
async function pasteToken(provider, state, extraKey, label) {
  if (!state.token.trim()) return showMsg('Vui lòng nhập Access Token', 'err')
  if (!state[extraKey]?.trim()) return showMsg(`Vui lòng nhập ${extraKey === 'shop_id' ? 'Shop ID' : extraKey === 'advertiser_id' ? 'Advertiser ID' : 'Ad Account ID'}`, 'err')
  state.saving = true
  state.info = null
  try {
    const extra = { [extraKey]: state[extraKey].trim() }
    ...
    const ext = r.data.long_lived ? ' — Token đã gia hạn 60 ngày' : ''
    showMsg(`✓ Kết nối ${label} thành công · Tài khoản: "${r.data.name}"${ext}`)
  } catch (e) {
  ...
  <div>
    ...
    <div class="ds-card">
      ...
      <div class="token-form">
        ...
        <textarea v-model="fb.token" rows="3" placeholder="EAAxxxxxxxxxxxxxxxx..." class="mono" />
        <div class="step-row">
          <span class="step">3</span>
          Ad Account ID (tìm tại <a href="https://business.facebook.com/settings/ad-accounts" target="_blank" class="ext-link">Business Settings ↗</a>):
        </div>
        <div class="inline-row">
          <input v-model="fb.ad_account_id" placeholder="act_123456789" class="mono" />
          <button class="btn btn-gold" :disabled="fb.saving || !fb.token.trim()" @click="pasteToken('facebook', fb, 'ad_account_id', 'Facebook')">
            <span v-if="fb.saving" class="spinner" />{{ fb.saving ? 'Đang xác thực…' : 'Lưu & Xác thực' }}
          </button>
        </div>
        <div v-if="fb.info" class="verify-ok">
          ✓ <strong>{{ fb.info.name }}</strong>
