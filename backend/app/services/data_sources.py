...
App credentials đọc từ Settings (env vars) — không cần DB config.
"""
...
def get_meta_ads_cost(db: Session, user_id: int, since: str, until: str) -> list[dict]:
    """Chi phí Meta Ads theo ngày [{date, spend}]."""
    ...
    s = get_settings()
    ad_account_id = s.FACEBOOK_AD_ACCOUNT_ID
    if not ad_account_id:
        raise ValueError("Chưa cấu hình FACEBOOK_AD_ACCOUNT_ID trong .env")
...
def get_shopee_revenue(db: Session, user_id: int, since: str, until: str) -> list[dict]:
    ...
    return ShopeeClient.from_settings().fetch_revenue(token, int(shop_id), time_from, time_to)
...
def get_tiktok_ads_cost(db: Session, user_id: int, since: str, until: str) -> list[dict]:
    ...
    advertiser_id = extra.get("advertiser_id") or get_settings().TIKTOK_ADVERTISER_ID
    if not advertiser_id:
        raise ValueError("Chưa cấu hình TIKTOK_ADVERTISER_ID trong .env")
...
def get_google_ads_cost(db: Session, user_id: int, since: str, until: str) -> list[dict]:
    ...
    if not customer_id:
        raise ValueError("Chưa cấu hình GOOGLE_ADS_CUSTOMER_ID trong .env")
    if not developer_token:
        raise ValueError("Chưa cấu hình GOOGLE_ADS_DEVELOPER_TOKEN trong .env")
    # Tự động refresh nếu có refresh_token
    if conn and conn.refresh_token:
        try:
            refresh_tok = decrypt_secret(conn.refresh_token, s.SECRET_KEY)
            token = google_client.refresh_access_token(refresh_tok)
        except Exception:
            pass
    return google_client.fetch_ad_spend(token, customer_id, since, until, developer_token, login_customer_id)
